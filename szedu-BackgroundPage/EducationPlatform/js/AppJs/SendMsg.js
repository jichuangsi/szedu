layui.use(['form', 'table', 'element', 'xmSelect'], function() {
	...
	function getTeacherSysMsg() {
		...
		if(arr.length > 0) {
			$.each(arr, function(index, item) {
				...
				str += '<span onclick="quickReply(this)">回复</span>'
				str += '</div>';
			...
			$('#studentMsg').append(str);
		} else {
			$('#studentMsg').html("<div style='color: darkgrey;margin-top: 10px;text-align: center;'>暂无信息</div>");
			return layer.msg('近期暂时无信息');
		}
	}
	//确认已读
	...
	function getSysMsg() {
		...
		if(arr.length > 0) {
			...
			$('#studentSysMsg').append(str);
		} else {
			$('#studentSysMsg').html("<div style='color: darkgrey;margin-top: 10px;text-align: center;'>暂无信息</div>");
	...
	function getTeacherClass() {
		var url = '/teacherInfo/getAllStudent';
		var arr = studentGetMethod(url);
		$.each(arr, function(index, item) {
			classList.push({
				name: item.className,
	...
	function getTeacherColleague() {
		var url = '/teacherInfo/getStudentTeacher';
		var arr = studentGetMethod(url);
		$.each(arr, function(index, item) {
			Colleague.push({
